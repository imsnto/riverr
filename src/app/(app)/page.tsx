...
function DashboardComponent() {
  const { appUser, userSpaces, setUserSpaces, activeSpace, setActiveSpace } =
    useAuth();
  ...
  const [rightPanelView, setRightPanelView] = useState<
    "threads" | "thread" | "task-from-thread" | "mentions" | null
  >(null);
  ...
  const [channelLastReadAt, setChannelLastReadAt] = useState<
    Record<string, number>
  >({});
  const [threadReadAt, setThreadReadAt] = useState<Map<string, number>>(
    new Map()
  );
  ...
  const isThreadUnread = React.useCallback(
    (parent: Message) => {
      ...
      const repliesFromOthers = messages.filter(
        (m) =>
  ...
  const userInvolvedThreads = React.useMemo(() => {
    ...
    return messages.filter((parent) => {
      ...
      if (!parent.reply_count || parent.reply_count === 0) return false; // no replies, then not a thread

      const ch = channels.find(
        (c) => String(c.id) === String(parent.channel_id)
      );
      if (!ch || String(ch.space_id) !== String(activeSpace.id)) return false;

      const allMsgsInThread = [
        parent,
        ...messages.filter((m) => String(m.thread_id) === String(parent.id)),
      ];
      return allMsgsInThread.some(
        (m) => String(m.user_id) === String(appUser.id)
      );
    });
  ...
  const getUnreadMentions = useCallback(() => {
    ...
    const messageMentions = messages.filter(
      (m) =>
    ...
    const taskMentions = tasks.flatMap((t) =>
      (t.activities || [])
        .filter(
          (a) =>
            a.comment &&
            mentionRegex.test(a.comment) &&
            String(a.user_id) !== String(appUser.id) &&
            new Date(a.timestamp).getTime() > checkTime
        )
        .map((a) => ({
          ...a,
          parentType: "task",
    ...
    const docMentions = documents.flatMap((d) =>
      (d.comments || [])
        .filter(
          (c) =>
            mentionRegex.test(c.content) &&
            String(c.userId) !== String(appUser.id) &&
            new Date(c.createdAt).getTime() > checkTime
        )
        .map((c) => ({
          ...c,
          parentType: "document",
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const allUserProjects = await Promise.all(
          userSpaces.map((s) => getProjectsInSpace(s.id))
        ).then((p) => p.flat());
        const allProjectIds = allUserProjects.map((p) => p.id);

        const [fetchedTimeEntries, fetchedSlackLogs, fetchedMessages] =
          await Promise.all([
            ...
            activeSpace
              ? Promise.all(
                  normalizedChannels.map((c) => getMessagesInChannel(c.id))
        ...
        toast({
          variant: "destructive",
          title: "Error",
          description: "Could not load workspace data.",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  const handleUpdateActiveSpace = async (updatedData: Partial<Space>) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Update failed",
        description: "Could not save space changes.",
      });
    }
  };
  ...
  const handleUpdateProject = async (
    ...
    setProjects((prev) =>
      prev.map((p) =>
        p.id === projectId ? ({ ...p, ...projectData } as Project) : p
      )
    );
  ...
  const handleRemoveTask = (taskId: string) => {
    ...
    dbDeleteTask(taskId).catch(() => {
      toast({
        variant: "destructive",
        title: "Delete failed",
        description: "Could not delete task from DB.",
      });
  ...
  const handleInvite = async (
    values: Omit<Invite, "id" | "token" | "status">
  ) => {
    if (!appUser) return;
  ...
  const handleCreateTask = async (taskData: Omit<Task, "id">) => {
    ...
    if (newTask && activeThread) {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.id === activeThread!.id ? messageWithTask : m))
      );
    }
  };

  const handleSaveChannel = async (
    channelData: Omit<Channel, "id">,
    channelId?: string
  ) => {
    if (channelId) {
      ...
      setChannels((prev) =>
        prev.map((c) =>
          c.id === channelId ? { id: channelId, ...channelData } : c
        )
      );
    ...
    switch (view) {
      ...
      case "messages": {
        ...
        const channelMembers = activeChannel
          ? allUsers.filter((u) => activeChannel.members.includes(u.id))
          : [];
        const simplifiedProjects = projects
          .filter((p) => p.space_id === activeSpace?.id)
        ...
        const leftPanel = (
          <div className="flex h-full flex-col">
                ...
                onClick={() => {
                  ...
                  if (opening) {
                    ...
                    setThreadReadAt((prev) => {
                      ...
                      userInvolvedThreads.forEach((t) =>
                        next.set(String(t.id), now)
                      );
                      return next;
                    });
                  }
            ...
            <div className="space-y-1 p-2 flex-1 overflow-y-auto">
              {channels
                .filter((c) => String(c.space_id) === String(activeSpace?.id))
                .map((channel) => {
                  const lastRead = channelLastReadAt[String(channel.id)] ?? 0;

                  const parentUnreadRaw = messages.filter(
                    (m) =>
                      String(m.channel_id) === String(channel.id) &&
                      !m.thread_id &&
                      String(m.user_id) !== String(appUser?.id) &&
                  ...
                  const parentUnread =
                    String(channel.id) === String(activeChannelId)
                      ...
                      : parentUnreadRaw;
                  const threadUnread =
                    unreadThreadsByChannel[String(channel.id)] || 0;
                            ...
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  ...
                                  permanently delete the #{channel.name} channel
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction
                                  onClick={() =>
                                    handleDeleteChannel(String(channel.id))
                                  }
                                  className={cn(
          ...
          <>
            ...
            {rightPanelView === "task-from-thread" && activeThread && (
              <CreateTaskFromThreadDialog
                ...
                channelMembers={channelMembers.map((u) => ({
                  id: u.id,
                  name: u.name,
                }))}
      ...
      case "flows":
        const renderFlowsContent = () => {
          switch (flowsView) {
            case "job_flows":
              return (
                <JobFlowBoard
                  activeSpace={activeSpace!}
                  allUsers={allUsers}
                  jobFlowTemplates={jobFlowTemplates.filter(
                    (t) => t.space_id === activeSpace!.id
                  )}
            ...
            case "templates":
              return (
                <div className="p-4">
                  <JobFlowTemplateBuilder
                    templates={jobFlowTemplates.filter(
                      (t) => t.space_id === activeSpace!.id
                    )}
                    phaseTemplates={phaseTemplates.filter(
                      (t) => t.space_id === activeSpace!.id
                    )}
                    allUsers={allUsers}
              ...
              return (
                <div className="p-4">
                  <PhaseTemplateBuilder
                    templates={phaseTemplates.filter(
                      (t) => t.space_id === activeSpace!.id
                    )}
                    allUsers={allUsers}
                    taskTemplates={taskTemplates.filter(
                      (t) => t.space_id === activeSpace!.id
                    )}
                    activeSpaceId={activeSpace!.id}
                    onSave={async (data) => {
            ...
            case "tasks":
              return (
                <div className="p-4">
                  <TaskTemplateBuilder
                    templates={taskTemplates.filter(
                      (t) => t.space_id === activeSpace!.id
                    )}
                    allUsers={allUsers}
                    activeSpaceId={activeSpace!.id}
        ...
        const isTemplateView = ["templates", "phases", "tasks"].includes(
          flowsView
        );
        return (
          <div className="flex flex-col h-full p-4 md:px-8 md:pt-8">
            <div className="flex items-center p-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  <DropdownMenuItem onClick={() => setFlowsView("templates")}>
                    Flow Templates
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setFlowsView("phases")}>
                    Phase Templates
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setFlowsView("tasks")}>
                    Task Templates
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
            <div className="flex-1 overflow-auto">{renderFlowsContent()}</div>
          </div>
        );
      case "settings":
        return (
          <div className="flex gap-6 h-full p-4 md:p-8">
            <aside className="w-56 flex-shrink-0 border-r pr-6">
              ...
              <div className="space-y-1">
                ...
                <Button
                  variant={settingsView === "spaces" ? "secondary" : "ghost"}
                ...
                <Button
                  variant={
                    settingsView === "timesheets" ? "secondary" : "ghost"
                  }
                  onClick={() => setSettingsView("timesheets")}
                  className="w-full justify-start"
                >
                  Timesheets
                </Button>
