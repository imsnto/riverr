...
export default function AllThreadsView({
  threads,
  messages,
  allUsers,
  appUser,
  ...
  const sortedThreads = threads.sort((a, b) => {
    ...
    const lastReplyB =
      messages
        .filter((m) => m.thread_id === b.id)
        .sort(
          (x, y) =>
            new Date(y.timestamp).getTime() - new Date(x.timestamp).getTime()
        )[0] || b;
    return (
      new Date(lastReplyB.timestamp).getTime() -
      new Date(lastReplyA.timestamp).getTime()
    );
  });
    ...
    <div className="flex flex-col h-full min-h-0">
      <div className="p-4 border-b flex-shrink-0">
        ...
        <p className="text-sm text-muted-foreground">
          All your thread conversations in one place.
        </p>
      </div>
                ...
                {showUnreadSeparator && (
                  <div className="relative my-4 px-4">
                    ...
                    <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 flex items-center justify-center">
                      <span className="bg-card px-2 text-xs text-primary font-semibold">
                        Unread
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pl-6 border-l-2 ml-4 space-y-4">
                      {threadReplies.map((reply) =>
                        renderSingleMessage(reply, allUsers)
                      )}
                    </div>
                    <ReplyComposer
                      thread={thread}
